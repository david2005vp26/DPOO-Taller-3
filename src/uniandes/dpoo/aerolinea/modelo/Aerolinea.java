...
public class Aerolinea
{
    ...
    /**
     * Agrega un nuevo vuelo a la aerolínea
     * @param vuelo
     */
    ...
    public Collection<Tiquete> getTiquetes( )
    {
        ...
        for (Vuelo vuelo : vuelos) {
        	tiquetesTotales.addAll(vuelo.getTiquetes());
        }
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        for (Vuelo vuelo : vuelos) {
            if (vuelo.getFecha().equals(fecha) && vuelo.getAvion().equals(avion)) {
                throw new Exception("El avión " + nombreAvion + " ya tiene un vuelo programado en la fecha " + fecha + ".");
            }
        }
    ...
    public int venderTiquetes(String identificadorCliente, String fecha, String codigoRuta, int cantidad) throws VueloSobrevendidoException, Exception {
        ...
        int tiquetesDisponibles = vuelo.getAvion().getCapacidad() - vuelo.getTiquetes().size();
        if (tiquetesDisponibles < cantidad) {
            throw new VueloSobrevendidoException(vuelo);
        }

        int costoPorTiquete = cliente.calcularValorTotalTiquetes(); 
        int costoTotal = costoPorTiquete * cantidad;

        for (int i = 0; i < cantidad; i++) {
            String codigoTiquete = GeneradorTiquetes.generarTiquete(vuelo, cliente, costoPorTiquete).getCodigo(); 
            Tiquete tiquete = new Tiquete(codigoTiquete, vuelo, cliente, costoPorTiquete);
